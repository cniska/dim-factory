...
| Factory | `factory-order.ts`, `order-ready.ts`, `queries.ts` | One row per piece of work, waiting or worked, and the evidence of what it produced |
...
- Read `order-ready.ts` for the order waiting work comes back in, and `factory-order.ts` for what a claim, a stop and the completion gate hold.
