...
| Factory | `factory-order.ts`, `queue-store.ts`, `queries.ts` | Keep what a queue wants made separate from what an order did about it |
...
- Read `queue-store.ts` for dependency readiness, the order ready items come back in, and status transitions.
