...
Use this shared capability after a slice is green and before its checker reads it. Read the slice diff, not the surrounding codebase, and edit only where the reader's cost falls without changing behavior.
...
State whether the pass changed code, which reader cost each edit lowered, and which check proved behavior was preserved. The build station then sends the resulting slice to its read-only checker.
