...
- **Was this already decided?** A decision already taken is not yours to re-take. `dim q chain <id-prefix>` and `dim q resume <id-prefix>` say whether this is mid-chain with a Next already written.
...
Hand off to [`dim-build`](../dim-build/SKILL.md) and follow it, one slice at a time: the repo's own task at the end of each, one checking agent on the slice's diff, then the commit, then the next.
