...
**Hand the order to a builder**, at the tier `dim route builder` gives you. Pass the order, base revision and repo check; the builder's claim already made its isolated worktree, so the builder `cd`s there, invokes the station itself and takes the order end to end. Running the station in this session makes the line one worker long, and then the queue is worked one item per invocation whatever this file says about emptying it.
...
**Minting is the builder's first act, before it reads anything.** A builder the factory started as a process carries its worker in the environment it was started in; one running inside the operator's own process has none, mints its own, and until it does every write it tries is refused — so the order reads as idle while the work happens. Say so in the brief, and check the first hand-back for a worker row rather than trusting that it did.
...
- **The worker comes from the environment, never from a flag.** `DIM_WORKER_NAME` and `DIM_WORKER_TOKEN` are what the factory hands a worker it starts, and every `dim order` write reads them and records that name on the moment. A shell that was started by nothing becomes a worker with `eval "$(dim worker mint --role operator)"`, naming the role it acts in — this file runs the line, so it is the operator; without one, every write is refused rather than recorded against nobody.
