...
- Ensure the operator identity exists with `eval "$(dim worker register --role operator)"`; the command uses the current harness session recorded by its startup hook when no session variable is exposed.
