...
The declared half is built and delivered. `src/tasks.ts` reads `package.json` scripts through the package manager the lock file names, `mise` tasks and `Makefile` targets, and `checkTask` picks the widest check a repo declares — `verify` over `test`, because gating on tests alone passes a change that does not compile.
