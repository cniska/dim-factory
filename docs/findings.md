...
## What Claude Code does when a station runs it headless

Probed on 2026-09-25 against Claude Code 2.1.282, building the Claude harness. Each is one run on one machine, so it says what this version did and not what every version does.

- Bash inside the sandbox enabled through `--settings` ran without a prompt under `--permission-mode acceptEdits` and under `default`. A write under the working directory or an `--add-dir` succeeded; a write to `$HOME` failed with `Operation not permitted`; with `filesystem.denyWrite` on the working directory, a write there failed the same way. `acceptEdits` works in `-p`, although a docs summary said it does not.
- `--settings '{"env":{"ANTHROPIC_API_KEY":""}}'` overrode an API key a project-local settings file set: the `init` event's `apiKeySource` went from `ANTHROPIC_API_KEY` to `none`, and the run completed on the signed-in plan. What `apiKeySource` reports for a login made with `claude setup-token` was not probed.
- Sandboxed Bash could write anywhere under the session's `$TMPDIR` whatever the settings said, so a write probe placed there proves nothing; the probes below that concern git ran in a repository under `$HOME`.
- In a linked worktree, sandboxed Bash could write the repository's common git dir — a new directory under `worktrees/` — although only the worktree's own git dir and the common dir's `objects`, `refs` and `logs` were passed with `--add-dir`. Claude grants that dir on its own, so what a builder must not write there has to be denied rather than left out.
- `sandbox.filesystem.denyWrite` and an `Edit(...)` permission deny both honored a `*` glob: `<common>/worktrees/*/commondir` refused a write to a worktree created after the run started, and refused the Write tool on another worktree's `commondir`, while commits in the builder's own worktree still went through.
- `claude --resume <id>`, run from a different working directory than the session started in, kept the session id and the conversation.
- With `--json-schema`, the `result` event carried the answer both as `result` text and as `structured_output`, while the last assistant text before it was the prose `ok`.
