...
Before tasking review, the operator checks the builder's exact latest commit and passing check. The review command refuses to spawn without that evidence, so a clean process cannot mistake a builder's successful exit for an outcome the operator has checked. A later commit replaces the review target. The owner approves the final Build artifact before review, then approves the Review artifact before shipping.

After a review closes, the operator approves that exact Review artifact before the order can ship. Approval follows every finding the order has had, not only the latest round's ([`src/order-finding-state.ts`](../src/order-finding-state.ts)). It is refused with `finding_unsettled` while any finding is open — raised and not yet ruled addressed, ruled `not_addressed`, or a refusal the owner overturned — and with `ruling_pending` while a refusal the reviewer contested waits on the owner. An aborted review, or a round that raised a finding or ruled one `not_addressed`, returns the order to the builder loop. A round that only contests a refusal holds the order for the owner, whose word the operator records with `dim order rule <finding-id> --uphold|--overturn --reason "..."`: an upheld refusal is settled, and an overturned one returns to the builder as work. The builder's brief lists under `# Review findings`, by id, every open finding with no answer since its last ruling — one a round raised, a fix a round ruled `not_addressed`, and a refusal the owner overturned — each with its location, failure, fix and whichever of the owner's and the reviewer's reasons sent it back. The build turn answers each of them, `fixed` or `refused` with a resolution, and an overturned refusal takes only `fixed`; a finding answered again is answered by a later attempt, and the next round rules on its latest answer. A refusal already given that still stands goes under `# Refused findings` with the builder's own answer, marked as not work and left out of the turn's answers, for the next round to rule on. [`factory.md`](factory.md) says how the runner holds the turn to its answers. `dim order ship` itself requires the approved plan, the approved final Build artifact, the approved review and the operator's identity, so a worker cannot advance its own outcome.
