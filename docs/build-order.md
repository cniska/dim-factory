...
- **`withoutWorktree` folds a repo root to a key belonging to no repo.** The fold splices the tail back on from the first `/` after the worktree name, which is right for a file path and wrong for a toplevel, where there is no tail: `/h/code/one/.claude/worktrees/wt` becomes `/h/code/onewt`. `convention` is the only caller applying it to `repo_commit.repo`; the others fold `f.path` and are correct. So a repo with no remote is split from its own worktree and each half judged against the floor alone — verbatim the failure the docstring says the fold prevents.
...
- **The wanted-hook list is kept by hand in two files.** `src/hooks.ts` names the three event-and-command pairs it installs and `src/codex-trust.ts` names the same three to find their trust keys, along with its own copy of the entry type and the `hooks.json` path. Adding or renaming a hook in one and not the other does not fail: `planCodexTrust` simply stops reporting the new hook, which reads exactly like a hook that is trusted. Exporting the list from `hooks.ts` is the cut; the two files are already coupled, since the trust test imports `hookCommand`.
