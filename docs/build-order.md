...
- **A workspace and worker-environment contract.** A profile resolves the checkout, worktree, languages, package managers, workspace members, declared setup entry point, check, format and test tasks, required services, isolation strategy, cleanup entry point, and resource or secret requirements. `dim wt` invokes the repository-owned setup and teardown hooks, records their structured reports, and keeps a worktree when cleanup fails. Package installation, containers, ports, environment files and service health remain repository-owned side effects; the factory observes them and attaches them to the job lifecycle.
