...
- **A station for simplification**, designed in [`design.md`](design.md): a third front door beside `dim-feat` and `dim-fix`, whose phase 1 is the aiming. The tool-agnostic skill is the second most loaded on this machine, and the files it is called on arrive drawing fixes at four times the rate of files arriving at `build` ([`findings.md`](findings.md)) — so the station is that aiming made deliberate, `q fixes` and `q rework` for what has come back and `q exemplars` for what to leave alone, rather than a reading of whatever is open. Two pieces follow it rather than block it: the unmodified-tests rule as a hook, and a catalog of smells as one agent per smell with a fixed brief, of which only file length, parameter count and duplicate blocks are shapes a gate can hold.
