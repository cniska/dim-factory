...
- **A `doctor` check that a hook is trusted, not merely installed.** `doctor` reports the Codex hooks as installed when they are written to `~/.codex/hooks.json`, but Codex runs one only if `~/.codex/config.toml` records a `trusted_hash` for it under `[hooks.state]`, keyed `<hooks.json path>:<event>:<entry index>:<hook index>`. Trust is therefore positional: another tool inserting an entry ahead of dim's shifts the index and silently un-trusts it, so collection and `wake` stop with nothing reporting it. This is the silent-failure class `doctor` exists for.
- **The rest of the agent-facing output.** Numbers no longer carry a thousands separator, but every cell is still padded to the widest value in its column, and a result over forty rows prints `… N more rows` without naming the flag that widens the window. Both cost an agent tokens or data ([`design.md`](design.md)).
