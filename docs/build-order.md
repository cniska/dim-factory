...
- **A read-only factory wall.** [`human-interface.md`](human-interface.md) defines a local Bun-hosted React page using shadcn/ui and Tailwind, with a dark, compact visual system and an operational wall hierarchy. It reads the existing factory records, sends changed snapshots over a loopback WebSocket, and gives the owner high-level visibility into active work, attention items, next work and recent outcomes. It has no control endpoints and does not add a second state store.
