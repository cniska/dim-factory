...
- **A workspace and worker-environment contract.** [`factory.md`](factory.md) defines the profile. `dim wt` invokes the repository-owned setup and teardown hooks and keeps a worktree when cleanup fails; package installation, containers, ports, environment files and service health remain repository-owned side effects. A running job may attach a setup or teardown report to `factory_job` evidence, where `dim q job` reads it back. [`src/workspace.ts`](../src/workspace.ts) resolves the checkout, worktree, languages, package managers, workspace members, declared tasks and the two hook entry points; the required services, the isolation strategy and the resource or secret requirements are what the profile still has to name.

- **What the factory wall still cannot show.** The board is live: `dim wall` serves a three-column lifecycle page over a loopback WebSocket, with no control endpoints and no second state store ([`human-interface.md`](human-interface.md)). Three things it cannot say yet. A worker's name is derived by [`src/worker-name.ts`](../src/worker-name.ts) from its agent id, so editing that word list silently renames every worker that ever ran — a durable `agent_id → display_name` table is what makes a name a fact rather than a function. A card names no project, so one board cannot carry work from more than one repo. And no operator record exists at all, so the wall cannot say which harness is driving the floor; [`factory.md`](factory.md) designs that boundary and nothing implements it.

- **The self-sufficient factory job.** [`factory.md`](factory.md) defines the job contract: one job owns one queue item in one worktree, may delegate internal work, records bounded lifecycle and evidence, and stops at completion or a recorded terminal outcome. `runFactoryJob` enforces the single worktree assignment, exposes delegation and stop operations, records builder failures, and rejects lifecycle or evidence writes after the terminal boundary. `dim q factory [job-id-prefix]` reads the unified current-status row with selected evidence, while `dim q job <job-id>` reads the detailed event report. Queue selection, serialized integration and queue-state enforcement remain for the orchestration slices. Independent jobs can run in parallel, while claims, integration and queue transitions stay serialized. The explicit item count is the run boundary and defaults to one.

- **A harness-neutral factory scheduler.** [`factory.md`](factory.md) defines the boundary: `dim` owns schedule state, due selection and run evidence, while Codex, launchd, cron or another host only invokes it. The first slice is live as persisted interval schedules with `dim schedule define|pause|resume` and `dim q schedules`; host installation, queue claiming and job execution evidence remain outside it.

- **The owner's verdict on a finished job.** [`factory.md`](factory.md) argues it: a gate earns its automation on its own merit, and nothing today holds the merit. `factory_job_finding` records an agent's finding with the grounds a refusal rested on, and the owner's decision on a finished job has no equivalent row — landed as it came, sent back, or changed before landing, with the grounds whenever it was not the first. What it answers is which kinds of work stopped earning a reading, as a query rather than a memory of how the last few felt. It waits on jobs being recorded as work is taken, since a verdict attaches to a job.
