...
- **The self-sufficient factory lane.** [`factory.md`](factory.md) defines the planned contract: one lane owns one queue item in an isolated worktree, may delegate its internal work, runs to completion or a recorded stop, and leaves the driver to schedule, observe and integrate. Its delivered-product report carries the item and queue identity, lane/run/agent identity, station and delegation tree, worktree and branch, changed files, commit SHA, repo check and result, checker findings and resolutions, updated docs, final status, fence or blocker evidence, and timestamps. The report is persisted in the dim database and queryable after the session. Independent lanes can run in parallel, while claims, landing and queue transitions stay serialized. The explicit item count is the run boundary and defaults to one. The contract is not live until the report, ownership, claims and transitions are recorded and enforced.
...
- **`spansHistory` decides a default, and the query decides the filter.** The flag's whole effect is [`src/since.ts`](../src/since.ts) choosing whether the default window applies; an explicit `--since` still reaches `ctx.since` whatever it says. Whether the rows obey that window is a separate decision each query makes by threading `window(...)` into its own SQL, and nothing ties the two together — so a query can print a window its rows ignored, which `q findings` did until a checker ran it ([`findings.md`](findings.md)). Thirteen of the queries set the flag and each carries the imperative half by hand. A query declaring the column it windows on, with the framework threading the predicate and printing the line from one place, makes the disagreement unrepresentable; the queries whose window spans a join declare that they take none, and the line then says all time rather than echoing a bound nothing honored.
...
- **A slice cannot commit unchecked.** Argued by 2026-09-18, when a lane bypassed the gate with `DIM_SKIP_CHECK=1` where nothing refused it and only `q slices` said so afterward. A `pre-commit` hook knows the repo it runs in and not which slice it is looking at, so what makes a check countable has to be settled before a gate can hold it. Until it exists, whether a slice was checked is a question only watching answers. The read-only half of this pair is held: [`dim-build`](../skills/dim-build/SKILL.md) and [`dim-review`](../skills/dim-review/SKILL.md) both launch their agents without write tools, after a checker reverted a builder's fix on the same day.

- **`q slices` decides what a check is by matching seven substrings.** `dim check-task` reads the declared check from the manifest and is the authority, but nothing stores what it returns, so the query guesses. It guesses correctly for all 21 declaring checkouts here and matches neither `cargo test` nor `go test` ([`findings.md`](findings.md)), so this is a latent miss rather than a live one. Recording each repo's declared check at sync is what lets the query join the authority; a repo declaring none then reports as undeclared rather than as unchecked.
...
- **Whether the simplification pass in `dim-build` pays.** The step is built and every slice now carries it, so it spends on each one; nothing measures what it returns. What would answer it is an arm — slices run with the pass against slices run without — scored by whether a later fix commit came back to what each touched, which is the join `q fixes` reads by skill and would have to read by arm. Until then the step rests on the owner's own practice, which is a reason to build it and not a measurement.
