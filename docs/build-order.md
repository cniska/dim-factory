...
- **A JSONC writer for the tool configs.** `hooks.ts` round-trips a tool's config through `JSON.stringify`, so installing a hook discards the reader's own formatting, and a config carrying a comment fails outright rather than installing. `jsonc-parser` edits in place and leaves untouched bytes untouched. Split from the markdown writer below because this half has a caller today and that half does not.
