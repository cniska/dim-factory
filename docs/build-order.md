...
- **`wake` and `check-task` answer "what does this repo declare" from different directories.** `projectLine` reads the manifest at the checkout root, while `dim check-task` reads `process.cwd()` with no climb ([`src/cli.ts`](../src/cli.ts)), so the same question from a subdirectory gets a command from one and silence from the other. The `pre-commit` hook is unaffected, because git runs a hook at the toplevel. Closing it means a checkout-root helper both can import, which is a module of its own rather than an export from `wake.ts`.
