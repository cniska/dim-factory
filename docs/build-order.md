...
- **`q slices` counts scratch repos as work.** 88 of the commits it reports with no check in front of them are fixtures under `/private/tmp` from an earlier session's test runs ([`findings.md`](findings.md)). `isScratchRepo` already names those roots and already excludes them from what `sync` ingests; the query reads `session.project` and never asks it. One call, and the denominator stops carrying them.
