...
- **A simplification pass inside `dim-build`**, designed in [`design.md`](design.md): between the repo's task passing and the checking agent, scoped to the slice, so it fires because the loop fires rather than because someone remembered. The owner already runs the tool-agnostic skill this way by hand, which is the evidence the step is wanted; nothing measures whether it pays on fresh code, and that measurement is the first thing to want from it.
