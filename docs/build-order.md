...
- **The checking agent is read-only, and a slice cannot commit unchecked.** Two gates, both argued by 2026-09-18: a checker edited a builder's working tree and silently reverted its fix, caught only because the file tools report an on-disk change; and a lane bypassed the gate with `DIM_SKIP_CHECK=1` where nothing refused it and only `q slices` said so afterward. The first is a line in [`dim-build`](../skills/dim-build/SKILL.md) and its brief. The second is the harder one: a `pre-commit` hook knows the repo it runs in and not which slice it is looking at, so what makes a check countable has to be settled before a gate can hold it. Until both exist, whether a slice was checked is a question only watching answers.
