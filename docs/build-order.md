...
- **A workspace and worker-environment contract.** A profile resolves the checkout, worktree, languages, package managers, workspace members, declared setup entry point, check, format and test tasks, required services, isolation strategy, cleanup entry point, and resource or secret requirements. `dim wt` invokes the repository-owned setup and teardown hooks and keeps a worktree when cleanup fails. Package installation, containers, ports, environment files and service health remain repository-owned side effects; structured setup and teardown reports are available from `dim wt`, while attaching them to a factory job remains a separate persistence slice.
