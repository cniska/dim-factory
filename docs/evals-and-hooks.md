...
| An off-the-shelf eval runner | Not adopted. [promptfoo](https://www.promptfoo.dev/docs/configuration/reference/) (docs read 2026-09-18) would supply what `run.sh` lacks — `maxConcurrency` against a serial loop, `writeLatestResults` and a viewer, a wider assertion vocabulary. It supplies nothing for the constraint that actually binds: it documents `repeat` and no significance testing, while the limit here is a 40-point floor at k=5 (§2.4). Its `exec:` provider also "treats standard output as the response `output`, even when the script prints JSON", which discards the `.modelUsage`, `.num_turns` and `.total_cost_usd` fields §2.3 reads off `claude -p`. Revisit when wall clock and result storage are what hurt, which is a dozen scenarios away, not one. |
