...
| [Superpowers](https://github.com/obra/superpowers) | A plan review-focus pass for implied inputs or failure modes that slice tests do not cover | The workflow overlaps this repo's planning, test-driven slices and review stations. Its planning skill has a concrete check: name up to five inputs or failure modes implied by the spec but not exercised by task tests, and tie each to the owning slice's test. `dim` plans have checks and risks, but no explicit pass for uncovered spec implications; this is the strongest candidate to borrow. Its `diagnosing-superpowers` skill reports session failures with transcript line evidence, which overlaps `dim`'s durable order/session queries; its narrower value is diagnosing skill activation or compaction behavior. Native plan execution uses one final review, while this factory checks and reviews each slice, so its execution mode does not transfer. Read at [Superpowers v6.4.1](https://github.com/obra/superpowers/releases/tag/v6.4.1), commit `5bf4e78`. |
