...
- **Landing squashes.** `gh pr merge --squash` (`.codex/skills/land/SKILL.md:98`), so every commit the agent wrote is rewritten as it lands, while syncing is a merge and explicitly not a rebase (`.codex/skills/pull/SKILL.md:3-6`). This repo does neither: it lands by fast-forward, rebasing a branch the trunk has moved past at ship and recording each rewritten commit against the one it replaced ([`factory.md`](factory.md#done)). The squash it cannot take, since `reachesTrunk` tests a recorded sha for ancestry and a squash makes every order fail its own completion gate.
...
| Acolyte, this machine's own | A command contract, so each `dim` subcommand carries its own help | `acolyte/src/cli-contract.ts` is fifteen lines: a command is a handler plus a `help()` returning its own usage, and `cli-help.ts` prints the table rather than a written-out block. This repo does the same: a `Query` carries `name`, `summary` and `usage` and `q list` prints from that registry, and a `Command` carries the same three and `dim` alone prints from `src/commands.ts`. Take the contract and not the presentation: that CLI is read by a person who can try `--help`, read a README, or guess, and this one is read by an agent that takes the printed surface as the whole surface. A flag missing from the listing does not exist to it, which is what happened on 2026-09-18 when a query shipped with no `usage` and `q list` stopped offering an argument the README documents. So no banner, no examples block, no i18n — each costs tokens on every call and answers a question only a person asks. What is refused outright is most of that CLI: auth, oauth, credentials, cloud and daemon are out by the first invariant in [`AGENTS.md`](../AGENTS.md#invariants), and its `printError` is superseded by `src/warn.ts` and the lint rule behind it. Extends the borrow [`design.md`](design.md) already records. |
| [Superpowers](https://github.com/obra/superpowers) | A plan-review question: does the change imply an input or failure mode no slice's test exercises | The workflow overlaps this repo's planning, test-driven slices and review stations. Its planning skill has a concrete check: name up to five inputs or failure modes implied by the spec but not exercised by task tests, and tie each to the owning slice's test. Held as one question in the plan reviewer's brief (`skills/dim-station-plan/SKILL.md`) rather than a list the planner writes, so the plan is checked by judgement and the planner is told no count or format. Its `diagnosing-superpowers` skill reports session failures with transcript line evidence, which overlaps `dim`'s durable order/session queries; its narrower value is diagnosing skill activation or compaction behavior. Native plan execution uses one final review, while this factory checks and reviews each slice, so its execution mode does not transfer. Read at [Superpowers v6.4.1](https://github.com/obra/superpowers/releases/tag/v6.4.1), commit `5bf4e78`. |
