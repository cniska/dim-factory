...
|---|---|---|
| [CASS](https://github.com/jamesqo/coding_agent_session_search) | Reciprocal-rank fusion over the FTS5 and embedding scores, then a bounded rerank | `q search` scores by cosine alone and falls back to keywords. Fusing beats choosing, and [`findings.md`](findings.md) already records the failure mode — a narrow score band on a bad query — that a second signal separates. |
| [CASS](https://github.com/jamesqo/coding_agent_session_search) | Fingerprint sources and messages, embed only what changed | `dim embed` has no cursor of its own, so the byte cursor that makes `sync` incremental does not cover it. |
| [code-session-memory](https://github.com/djannot/code-session-memory) | Heading-aware chunking | A handoff is markdown with headings, and its `## Next` is the passage worth embedding. Their provider is remote and cannot be taken. |
