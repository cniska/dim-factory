...
- **Theme resolves meaning.** Semantic roles such as `builder`, `reviewer`, `planner`, `running`, `blocked` and `failed` are chosen by the layout and resolved by one fixed theme. Agent-role styling and order-state styling stay separate, so a role color never has to carry state as well.
