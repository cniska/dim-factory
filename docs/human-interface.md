...
- **Review.** The Review artifact the factory renders from the reviewer's report: the verdict — may advance, returns to the builder, or held for an owner ruling — then the blocking findings with their location, severity, failure and fix, each contested refusal with the builder's refusal beside the reviewer's reason, the rulings on earlier findings, plan conformance, every dimension's coverage, what was not judged, and at most three observations. A contested refusal is the owner's to settle, and the operator records that ruling with `dim order rule`; the tab stays read-only. It appears once review has been delegated.
