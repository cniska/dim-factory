...
- **Human words, machine ids.** The same split the board follows: the item's title leads, and the job id, item id, branch, worktree and commit shas are present for an agent to join on. A sha is shortened to what a person compares and carries the whole of itself for anything that reads the page.
