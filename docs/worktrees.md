...
It now ships as [`scripts/wt`](../scripts/wt) with its suite beside it, and `bun run verify` runs that suite, so the script and the tests move together. `dim install-wt --write` links it onto PATH rather than copying it — a copy on PATH and a copy in the repo are the drift it was adopted out of — and renames whatever was there to a backup, because for a real file that is the only copy of it on the machine. `dim doctor` warns when what runs is not the script the tests cover.
...
Undoing an agent's writes. `wt` is adopted and linked.
