...
**Transcripts.** `~/.claude/projects/<slug>/<session-id>.jsonl`, one JSON object per line; subagents at `<session-id>/subagents/agent-<id>.jsonl` with a sibling `agent-<id>.meta.json`. An agent id repeats across parent sessions, so a subagent's session is recorded as `<agent id>@<parent session id>`: keyed on the agent id alone, two subagents would be one row and the second would take the first's read cursor. 308 main files (1.17 GB) and 420 subagent files. Reference file for line citations below: `~/.claude/projects/-Users-christofferniska-code-skills/289e4680-8adc-4434-89f7-175277609c09.jsonl` (called **R** here; 558 lines).
...
[`src/schema.ts`](../src/schema.ts) is the schema. It is the file that creates the tables, so it is the only place the columns are stated, and each table carries the reason for its own shape beside it — why the source-file cursor is keyed by session rather than path, why no column adds the two tools' token counts together, why the tables with no source to re-read are the ones `rebuild` must not clear.
...
**Every table is rebuilt by re-reading its sources**, which is why a schema change is `dim rebuild` rather than a migration. `rebuild` drops each of those tables and recreates it from `SCHEMA_SQL`, which is what lets an added column arrive: `CREATE TABLE IF NOT EXISTS` leaves a table that already exists alone. The exceptions are the ones with no source to re-read — a hook fires once and the event is gone — and they say so at the table. `guidance_walk`, `trace_event` and `finding` are never cleared, and neither is `embedding`, whose vectors only the model can produce again. `correction_label`, `hook_event` and the factory order records are dropped anyway and written back row for row, because they hold judgements and claims that must still be able to take a schema change. `SCHEMA_VERSION` exists so `sync` refuses to run against a database only a re-read can correct, and `rebuild` stamps the new version only once that re-read has returned, so a run that threw partway leaves the database refused rather than waved through. A transcript that failed to ingest does not hold the stamp back: the tables have the shape the version claims, and `sync` reports the file that failed.

**The version is bumped for a change only a re-read can correct**: a changed column, or a changed rule for what identifies a row, since rows already written keep the old identity. Adding a table is neither, because every write opens the database through `SCHEMA_SQL`, which creates it. That exemption lasts only until some database has run the statement: from then on `CREATE TABLE IF NOT EXISTS` leaves the table as it is, so changing one of its columns strands that database. Whether a table is new is a fact about the databases on disk rather than about the diff — a wall reloading mid-edit can run a statement before it is committed — so a column changed after its statement has run anywhere takes a bump ([`findings.md`](findings.md) has the case). [`src/schema-version.test.ts`](../src/schema-version.test.ts) pins the version together with a digest of `SCHEMA_SQL`, so every schema edit changes that one line, and two branches editing the schema conflict there rather than merging into one version number.

**A finding is keyed on the repo and the file, not on the session.** `dim finding` runs inside the session it records and cannot name it, and the question the table exists for is whether a later fix came back to code a reviewer flagged — which `file` answers through `commit_file` and a session id does not. What it may never be read as is a score on the builder: the arm it supports asks whether checking pays, and [`findings.md`](findings.md) records measures that looked like grades and turned out to track what was being worked on.
...
**Concurrency**: hooks never open the database. One file per event (`O_CREAT` of a unique name) is atomic on APFS; the scheduled `sync` moves spool files into `hook_event` and derives `session.ended_at/end_reason` under the factory lock. Concurrent sessions therefore never contend on `sessions.db` through their hooks. `sync` writes it under that lock, which also keeps `rebuild`, `embed` and a ship from running beside it, but it is not the only writer: the `dim` commands that open the database outside the lock, and the trace a command leaves behind, each open their own connection and overlap with it and with each other, in WAL mode so the read path can query during a write. SQLite holds one write lock at a time, so a connection `openDb` makes waits a bounded time for it before failing with `SQLITE_BUSY`, and a trace waits a shorter time and drops its row ([`src/db.ts`](../src/db.ts), [`src/trace.ts`](../src/trace.ts)). The wait covers a statement that takes the write lock before reading. SQLite does not wait on behalf of a transaction that has already read: one that reads and then writes while another connection holds the lock fails with `SQLITE_BUSY` at once.
...
| `search "<question>"` | distilled passages closest in meaning: score, kind, when, ref, where, text — where a ref is a commit sha or `<session>@<ts>`, and falls back to the keyword index |
...
**A finding is answered, and the answer is read.** A reviewer that reports once leaves the builder to act or not with nothing looking again, so the last word belongs to whoever wrote the fix — and that fix is the part of a slice nothing has read, written after the pass that would have caught it. Each finding therefore gets a fix or a stated refusal, and the diff that answers goes back to the reviewer with the brief it had the first time. The fixpoint is no unanswered finding rather than no findings: a reviewer asked whether more could be improved always says yes, and a refusal that cost the builder anything would make fixing whatever was raised the cheap way out. Whether a fix answers a finding is a judgement, so this is station text and not a gate; what a gate can hold — the subject, the repo's check, the push — it already holds.
...
**The read path is for the agent first.** The queries that count — `skills`, `tools`, `corrections`, `rework` — answer a question the owner asks about the corpus. The queries an agent reaches for mid-task are `search`, `keywords` and `thread`: find the sentence, then read the exchange around it, because the sentence that settled a question is rarely the one that matched. The first two find it over different populations — `search` ranks the passages a person distilled, `keywords` matches words across every message anyone said — so a decision that was settled in conversation and never written down is reachable by the words it was settled in. Until a query changes what an agent does in the session it is run in, it is a report, and reports are not what this is for.
...
**The window is a spend control, not a correctness rule.** Every query covers the last 30 days unless told otherwise, because guidance, projects and habits all changed across the corpus and a count over all of history describes a machine that no longer runs. The constant is arbitrary — the same species as a default turn cap — so `--since <n>d|YYYY-MM-DD` moves it and `--all` removes it, the window appears in the denominator line, and the counts a rate divides by are windowed with the rows. A query is left unwindowed where a window would hide its answer: a time series, since one narrowed to a month is not one; a query aimed at something the caller named — a session, a repo, a path, a term to search for — since the answer is the thing itself; and `running`, whose argument is the span it covers.

**Search is an index, not a scan.** `message_fts` is an FTS5 index over `message.text` with external content, so it stores no second copy and reads the text back through `message.rowid`; insert, update and delete triggers keep it level with the table, and `rebuild` drops it because an index left standing would point into the `message` table that is dropped and refilled under it. It holds prose only: a message with no text is a tool call or its result. Both queries that read it — `keywords` and `search`'s fallback — search what anyone said, so a reminder the harness injected into a session is left out, while what `origin.kind` marks as a coordinator's or a peer's message stays, because an agent wrote that and relaying it does not make it injected. Terms are quoted before they reach FTS5, so a branch name or a flag searches as the word it is rather than as `NOT` or a column filter. A row need not carry every term: `keywords` unions one match per term, ranks by how many terms matched, then by relevance, then by recency, and states on every row how many of the terms it carried. Words past a cap are dropped with the reader told, since an argument arrives from a file as readily as from a person and a common word still costs more to match than a rare one.
...
- `dim embed` drops a row whose source is gone, and the table carries no foreign key, because `rebuild` drops `message` and `repo_commit` before writing the same ids back.
...
- Where nothing is embedded, the model will not load, or the database predates the table, `search` answers from `message_fts` and says so in its denominator. A retrieval path that can break is one nobody relies on. It reads the same population `keywords` does, stated where `message_fts` is described above.

What it does not hold is raw conversation turns, which are the measured-worse input. Whether adding them helps is for the benchmark below, not an assumption.

**A ranking change is scored, not preferred.** `dim bench` reads `retrieval.jsonl` from the data directory rather than the repo, since a question that measures anything names the owner's own work and this repo is public — one question per line as `{"id", "query", "question", "relevant": [{"ref", "grade"}]}` — runs each through the query its `query` field names, and reports recall@k and nDCG@k per question and as a mean. A `grade` is how good an answer that row is, a finite number above zero, and it is the gain nDCG discounts by position; a row that answers nothing is left out rather than graded zero. A `ref` is a full commit sha, a session id, or one passage as `<session id>@<timestamp>`, each of which this record must hold; it is matched against the shortened form the query prints. A session or a passage must also name something a person distilled, since a turn nobody distilled is addressable and is never returned. A ref naming a session grades every passage in it, which is how a question about a session as a whole is asked. Every other rule — a unique id, at least one relevant row, a query that exists — refuses at load time naming the line, so it can be found by running it. The definitions live in `src/rank-metrics.ts` rather than being imported, so a corpus scored today compares with one scored a year from now.

Three things follow from scoring through the query rather than against the index. A ranking is only better if it is better through the door an agent uses, so the runner reads the rows a caller would see. Those rows name a ref at the width they print it, and which entity that ref names is the query's choice — `search` prints a sha for a commit and `<session>@<timestamp>` for a message, which is the argument `thread` takes, so the string a reader follows to the exchange is the string a corpus grades — and a question whose label names something no query prints is refused rather than scored zero forever. A decision is usually one passage rather than a whole session, so grading at the passage lets a session contribute more than one question. And a query that prints no ref at all cannot be scored: `keywords` names a session and a timestamp, which identify a message to a reader and not to a program, so a question asked of it is reported unscorable rather than quietly dropped.

The corpus lives beside the database and not in this repo. A question that measures anything names the owner's own work, and this repo is public; what ships here is the format, the metrics and the runner.
...
- Not a compliance reviewer for skill instructions; that is `evals/`.
...
[`build-order.md`](build-order.md) holds why each unbuilt piece is next, and the queue `dim order ready` reads holds each order's status and what it waits on, changing as work lands. Those two say what is true now; the sections above argue for the shape rather than track it.
