...
**A ranking change is scored, not preferred.** `dim bench` reads `retrieval.jsonl` from the data directory — one question per line as `{"id", "query", "question", "relevant": [{"ref", "grade"}]}` — runs each question through the named query it names, and reports recall@k and nDCG@k per question and as a mean. The definitions live in `src/rank-metrics.ts` rather than being imported, so a corpus scored today compares with one scored a year from now.
