...
**A ranking change is scored, not preferred.** `dim bench` reads `retrieval.jsonl` from the data directory — one question per line as `{"id", "query", "question", "relevant": [{"ref", "grade"}]}` — runs each through the query its `query` field names, and reports recall@k and nDCG@k per question and as a mean. A `grade` is how good an answer that row is, a finite number above zero, and it is the gain nDCG discounts by position; a row that answers nothing is left out rather than graded zero. A `ref` is a full commit sha or session id that this record holds, matched against the shortened form the query prints. Every other rule — a unique id, at least one relevant row, a query that exists — refuses at load time naming the line, so it can be found by running it. The definitions live in `src/rank-metrics.ts` rather than being imported, so a corpus scored today compares with one scored a year from now.

Three things follow from scoring through the query rather than against the index. A ranking is only better if it is better through the door an agent uses, so the runner reads the rows a caller would see. Those rows name a ref at the width they print it, and which entity that ref names is the query's choice — `search` prints a sha for a commit and a session for a message, because a session is what the reader goes on to ask `thread` about — so a question is graded by what the query prints, and one whose label names neither is refused rather than scored zero forever. And a query that prints no ref at all cannot be scored: `keywords` names a session and a timestamp, which identify a message to a reader and not to a program, so a question asked of it is reported unscorable rather than quietly dropped.
