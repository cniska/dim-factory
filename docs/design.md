...
**Trigger**: a `launchd` agent (`~/Library/LaunchAgents/com.cniska.dim-factory.plist`, `StartInterval` 900) runs `dim sync`: archive changed files → ingest spool → ingest changed files. `flock` on `~/.local/share/dim-factory/lock` serializes runs. Hooks do not write the DB (§7). A manual `dim sync` does the same thing on demand; `dim backfill` is `sync` with the cursor reset.
