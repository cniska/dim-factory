...
- The oldest surviving main transcript is dated 2026-08-09 (`~/.claude/projects/-Users-christofferniska-code-hoodly/06ad4402-….jsonl`); the earliest `timestamp` inside any surviving transcript is `2026-07-22T08:29:39.659Z`.
- `~/.claude/history.jsonl` (32,471 prompt lines, earliest 2026-03-04) names 694 distinct `sessionId`s; 389 of them have no transcript on disk any more. Six project slugs under `~/.claude/projects/` are empty directories (`-Users-christofferniska-code-evolfy`, `…-festa`, `…-md2doc`, `…-md2gd`, `…-auto-clicker`, `…-acolyte--claude-worktrees-dependency-audit`): the sessions were cleaned up, the folders remain.
...
| **Usage is duplicated** | one API response is written as one `assistant` line per content block, each carrying the same `message.id` and the full usage | R has 156 assistant lines but 84 distinct `message.id`; corpus-wide 142,146 lines vs 81,620 ids. Summing per line overcounts by ~1.7x. Dedupe on `message.id`. |
...
| PR opened | `pr-link` line | `prNumber`, `prUrl`, `prRepository` — `…/-Users-christofferniska-code-hoodly/b5e7594e-….jsonl:269` |
...
| Skill listing (installed) | `response_item` `message` `role:"developer"` `<skills_instructions>` with roots `r0 = /Users/christofferniska/.agents/skills` | C:3 |
...
**Shared parsers or independent?** Independent, in bash + jq, and the owner has already said this work is separate from acolyte. The cost stated plainly: two codebases will track the Claude Code JSONL dialect and, once acolyte builds its Codex source, the Codex rollout dialect too; when either tool renames a field, both break and both are fixed separately, and the two will disagree in edge cases (acolyte treats a `<command-name>` prompt as noise; this treats it as a load). Reuse would cost more: `transcript-claude-code.ts` imports `./log`, `./task-activity`, `./tool-contract` (`:2-4`) and runs on Bun, so reusing it makes this project TypeScript and carries a port of those modules; and its zod schema (`:25-33`) exists to discard the fields this design keeps (`usage`, `model`, `attributionSkill`, `promptSource`, `toolUseResult`, sidechains), so "sharing" would mean growing acolyte's parser to a second product's requirements. The one thing worth copying rather than re-deriving is the harness-prompt prefix list (`transcript-claude-code.ts:64-75`), and even that is superseded here by `promptSource`/`origin.kind`, which acolyte's parser does not read. The `raw/` archive makes the duplication cheap to live with: a format change is fixed in one script and the DB is rebuilt.
...
- **Location**: `~/.local/share/session-db/` containing `sessions.db`, `raw/` (verbatim copies of transcript files), `spool/` (hook events), `schema.sql`. Home-directory path, `chmod 700`, never inside a repository; the collector's own repo (`~/code/session-db`) contains code and schema only, and `sessions.db` is not a path any repo tracks.
- **Pointer or archive?** Structure plus a pointer into the original files would keep the DB small and content-free, and it fails on the first of its two conditions: Claude Code prunes its files (§1: 30-day default, 389 sessions already gone). Codex does not prune (all 216 rollouts since 2026-02-05 are present) but it does move files — `archived_sessions/` holds 115 rollouts whose `threads.rollout_path` changed — so a path-based pointer breaks there too. Records are addressable stably in both formats: a Claude line by `(session_id, line_number)` in an append-only file (Claude Code's own later writes — `file-history-delta`, `compact_boundary`, `cost-state` — are appended lines, never rewrites; line numbers cited here stayed fixed across repeated reads during this study), a Codex line by `(thread_id, ordinal)` (C:1 `ordinal:0`). So: **archive the files, point into the archive.** Cost: the archive is a byte copy of the sources, 2.2 GB today (1.17 GB Claude, 1.04 GB Codex), growing at the sources' rate; privacy cost as stated below.
- **Raw layer = the archived files, not the DB.** `raw/claude/<slug>/<session>.jsonl` (+ `subagents/`), `raw/codex/<rollout filename>`, `raw/claude/history.jsonl`, `raw/codex/history.jsonl`. Copied whole when size or mtime changed (both formats are append-only; a shorter file than the archived copy is logged as an error, not overwritten; a Codex rollout that moves to `archived_sessions/` is matched by thread id in its filename, not by path). Every `message` and `tool_call` row carries `src_file` and `src_line`/`src_ordinal`, so a question that needs the full record — a tool result, a diff — re-reads one line from `raw/` by locator. This is what makes data captured now usable after any schema change: drop the DB and re-derive.
- **What the DB stores as text**: user prompts (typed/queued/human origin), assistant text, `userFeedback`, skill bodies' hash and size (not the body), Bash command strings (needed for commit/verify detection), file paths of edits. **Not stored**: tool results, file contents, `originalFile`/`structuredPatch`, `thinking` blocks, attachments, MCP results, stdout/stderr. Those stay in `raw/` only.
- **Privacy consequence, stated plainly**: `raw/` is a plaintext copy of every session, containing source, paths and whatever secrets a tool printed, and it removes the 30-day expiry that limited that exposure. That is the price of the analysis and of not losing the corpus; it is the same exposure class as `~/.claude/projects/` and `~/.codex/sessions/` today, with retention made indefinite. The DB itself is the narrower surface the read path touches. Exclude `~/.local/share/session-db/` from cloud sync tools; Time Machine is local and encrypted with FileVault, so it can stay.
...
CREATE TABLE source_file (
  path            TEXT PRIMARY KEY,     -- path under raw/
...
CREATE TABLE tool_call (
  ...
  src_line_result INTEGER,              -- where the result (stdout, diff) can be re-read from raw/
  extra           TEXT
);
...
**Boundary: no model anywhere in the collection path.** Ingestion is file copy, jq parsing and SQL inserts; backfilling seven months costs disk and CPU only. There is no pre-filter, scoring or relevance judgment at ingest — with no downstream scorer to defer to, storing everything and deciding at read time is the only policy that is not irreversible. A model enters only when the owner asks a question, and then it reads query output (aggregates, excerpts already in the DB), never transcripts. Skill detection is string and field matching on records that exist (§9.1); correction detection is deliberately left as candidate narrowing plus human or deliberately invoked judgment (§9.3).

**Implementation**: bash + jq + sqlite3 (3.51.0 on this machine), matching the repo's own eval harness. Each transcript line is one JSON object, so `jq -c` reads it as a stream; a 2 MB file is never held whole. jq emits TSV per target table; `sqlite3 .import` loads it into `TEMP` staging tables; one transaction moves rows with `INSERT … ON CONFLICT DO UPDATE`. Codex rollouts and Claude transcripts each get their own `ingest-<tool>.sh`; nothing is shared but `schema.sql` and `lib.sh` (paths, locking).
...
**Trigger**: a `launchd` agent (`~/Library/LaunchAgents/com.cniska.session-db.plist`, `StartInterval` 900) runs `session-db sync`: archive changed files → ingest spool → ingest changed files. `flock` on `~/.local/share/session-db/lock` serializes runs. Hooks do not write the DB (§7). A manual `session-db sync` does the same thing on demand; `session-db backfill` is `sync` with the cursor reset.
...
**Shape**: `session-db q <name> [--project <cwd>|--all-projects] [--since 30d] [--model <m>] [--tool claude|codex] [--skill <name>]`. Bash + sqlite3, each named query a `.sql` file under `queries/`, output as aligned text capped at 40 rows; `--json` for the eval export. Raw SQL stays available (`sqlite3 ~/.local/share/session-db/sessions.db`) for the owner; the agent uses only named queries. A TUI or dashboard is not built: the audience is one person and one agent, and 40 lines of text is the right size for both.
...
**Agent invocation**: a skill `session-evidence` in the skills repo (`skills/session-evidence/SKILL.md`), following the repo rules (frontmatter `name` matches directory, imperative description, terse body, `## See also` with bare names `skill-authoring`, `handoff`, `search-sessions`, `## Red flags`). It states the prerequisite (`session-db` on PATH) and two uses only: before editing a skill, run `q skill <name>` to see load frequency, versions and corrections; when writing a handoff or a skill-test report, run `q session <id>` for the factual summary. It earns its place narrowly — `skill-authoring` and `skill-test` are the sessions that need evidence — and if after a month the owner has not reached for it from an agent session, it is deleted and the CLI stays a human tool.
...
- Not a consumer of model calls at any point in collection or backfill; a model reads query output only when the owner asks, on a set the query has already narrowed.

## 12. Build order

Each slice is one commit in `~/code/session-db`, independently useful.

1. **Archive** — `session-db init` (directories, `schema.sql`, `source_file` only) and `session-db sync` archiving Claude transcripts, subagents, both `history.jsonl`, Codex rollouts and archived rollouts into `raw/`; launchd plist every 15 min; a `Makefile` with `install` (plist + hooks lines). Manual step documented in README: set `cleanupPeriodDays` to 3650. *Useful on its own: nothing is lost from this day on, and the 2026-07-22 → today Claude corpus is safe.*
2. **Hook spool** — the four hook commands added to `~/.claude/settings.json` and `~/.codex/hooks.json`; `sync` drains the spool into `session_event`. *Useful: session end reasons start accumulating immediately, and they cannot be backfilled.*
3. **Sessions, messages, usage, turns** — `ingest-claude.sh` and `ingest-codex.sh` populate `session`, `message`, `usage`, `turn`, `session_cost_reported`, `orphan_prompt`; `q tokens`, `q session`, `q models`, `q cost`. Backfill over `raw/`.
...
Slices 3–6 rebuild from `raw/` at any time; a schema change is `session-db rebuild`, not a migration.
