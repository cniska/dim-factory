...
- **It is a body of facts rather than a rule.** What an earlier session decided, what a number was, which version shipped. Facts are unbounded and a query is one line, so here the pointer wins outright. This is the whole of `df-sessions`.
