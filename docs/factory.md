...
**Partly built.** The harness-neutral request and event contract and deterministic fake scenarios live in [`src/harness.ts`](../src/harness.ts) and [`src/fake-harness.ts`](../src/fake-harness.ts). The external runner and Codex adapter remain to be built. A profile will validate capabilities, build the harness command and normalize its machine-readable events. Later adapters such as Claude, Cursor, Grok, Pi and OpenCode will implement that contract without changing station orchestration, worker identity or audit records. Profile discovery and operator selection per assignment come after the first adapter is proven; an unavailable or incomplete profile is refused rather than silently replaced.
