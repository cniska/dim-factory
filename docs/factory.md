...
**Partly built.** The harness-neutral request and event contract, deterministic fake scenarios, timeout runner and external process adapter live in [`src/harness.ts`](../src/harness.ts), [`src/fake-harness.ts`](../src/fake-harness.ts), [`src/harness-runner.ts`](../src/harness-runner.ts) and [`src/harness-process.ts`](../src/harness-process.ts). The Codex adapter remains to be built. A profile will validate capabilities, build the harness command and normalize its machine-readable events. Further adapters will implement that contract without changing station orchestration, worker identity or audit records. Profile discovery and operator selection per assignment come after the first adapter is proven; an unavailable or incomplete profile is refused rather than silently replaced.
