...
When a station process returns an explanation, a failed attempt carries that explanation in its reason so the operator can diagnose the hand without reopening the harness session. A retried Build receives the last failed Build reason in its brief so it can address the recorded failure.
