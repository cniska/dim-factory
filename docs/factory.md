...
The queue is an issue tracker in the database that supplies work to the operator without becoming the execution ledger.

- **Queue source.** `queue_item` and its dependency and transition tables own the item description, generated title, dependencies, priority and status, keyed on the repo's `owner/repo` so an item belongs to the project it is built in. Two orders adding items while they run is a write each rather than the lost write one file would take.
...
- **Queue format.** The rows are stated in [`src/schema.ts`](../src/schema.ts). Each item has a non-empty `id` unique to its queue, a `title`, an optional `description`, its dependencies, a priority and a current `status`. A transition records the status it came from and went to, when, an optional reason, and the order that caused it, so an item taken twice reads as two attempts rather than one. A dependency on an item that is not in the queue is rejected, as is an item depending on itself.
- **Lifecycle.** An item is `planned`, `claimed`, `completed` or `dropped`, which are the item's own states and not the order's: an order that stopped blocked, fenced or failed puts its item back to `planned`, and why it stopped is the order's to say. `completed` and `dropped` do not transition again.
- **Parallel work.** The planner identifies ready items whose dependencies are satisfied. Grouping them into independent isolated orders, claims and integration remain operator responsibilities.
...
- **CLI.** `dim queue ready [--limit <n>]` prints each ready item as JSON, most urgent first and oldest before newest within a priority; `dim queue add <item-id> --title "..."` puts one in, including from inside a running order; and `dim queue transition <item> <status> [--reason "..."] [--order <id>]` records it moving and which order moved it. All three default to the checkout's own `owner/repo`.
- **Serialization.** The operator keys claims and integration by queue and item, so work sharing a key is serialized while independent keys can run in parallel.
- **Boundary.** The queue owns intent and dependency state; `factory_order` owns execution identity, lifecycle events and evidence. The operator translates between them.
