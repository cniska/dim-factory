...
- **Work.** Station, every move between stations, each station attempt's start and finish, the worker and operator each attempt is attributed to, commit SHA, and each changed file with the lines added and removed where the recorder counted them.
...
The name is issued rather than discovered. A shell becomes a worker through `dim worker mint --role <r>`, which requires the harness session in `DIM_SESSION_ID`; the session is unique, so minting cannot create a second identity for that harness session. A station that starts a child first creates a one-use `dim worker assign --role <r>` assignment. The child harness receives that assignment, starts with its own `DIM_SESSION_ID`, and runs the internal `dim worker bootstrap <assignment-id>` protocol; only then does the factory create the worker and record the direct parent. The child receives no parent worker token, and no worker action is accepted before the child identity exists. The worker name and token then reach the process in `DIM_WORKER_NAME` and `DIM_WORKER_TOKEN`, with only the token digest stored. The token is what makes the name a claim only its holder can make, the issued set being readable through `dim sql`.
...
**Partly built.** The harness-neutral request and event contract, deterministic fake scenarios, timeout runner, external process adapter and Codex adapter live in [`src/harness.ts`](../src/harness.ts), [`src/fake-harness.ts`](../src/fake-harness.ts), [`src/harness-runner.ts`](../src/harness-runner.ts), [`src/harness-process.ts`](../src/harness-process.ts) and [`src/codex-harness.ts`](../src/codex-harness.ts). A profile will validate capabilities, build the harness command and normalize its machine-readable events. Further adapters will implement that contract without changing station orchestration, worker identity or audit records. Profile discovery and operator selection per assignment come after the first adapter is proven; an unavailable or incomplete profile is refused rather than silently replaced.
