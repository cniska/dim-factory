...
The queue planner is a reversible, file-backed issue tracker that supplies work to the factory operator without becoming the execution ledger.
...
- **Boundary.** The queue file owns intent and dependency state; `factory_order` owns execution identity, lifecycle events and evidence. The operator translates between them.
