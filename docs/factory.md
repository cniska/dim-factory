...
Every moment on an order names a worker, in the column the moment is written with, under `NOT NULL`. An act with no worker is therefore not something the record declines to hold but something it cannot represent, which needs no hook to have fired and no check to have been remembered.

The name is issued rather than discovered. The factory mints a worker before it starts anything, so the identity exists before the first act and is unique by construction; it reaches the work in the environment the worker is started in, as `DIM_WORKER_NAME` beside a `DIM_WORKER_TOKEN` whose digest alone is stored. The token is what makes the name a claim only its holder can make, the issued set being readable through `dim sql`. `dim worker run --role <r> -- <argv>` is what issues one and starts the process carrying it, and a shell nothing started becomes a worker with `dim worker mint --role <r>`, so a person at a terminal is a worker too and no act is exempt.
...
Nothing is ever attributed afterwards. An audit entry completed later is a mutation of a record someone may already have read, and a log that can be amended is not evidence. What a harness calls its own agent is therefore a later-learned attribute of the worker rather than of any moment: a correlation that never arrives costs the log nothing, and a worker never observed as an agent reads as not observed. The failure this leaves is a worker that recorded nothing, which is true as it stands, in place of an act nobody can attribute, which is a hole that reads the same as work that never happened.
...
The self-sufficient order contract is live as `runFactoryOrder`: it claims one supplied item, marks it working, passes the item and base revision to a builder, and records the builder's terminal outcome and evidence through the existing factory tables. Assignment belongs to the factory: it starts the builder with a minted worker identity, and that worker makes the claim so the assignee on the first working event is the builder rather than the operator. A builder owns the order's worktree once and stops through `context.stop` or by returning one terminal outcome. Non-terminal lifecycle events and normalized evidence are accepted only while the order is working; a completed order must have a worktree, and no lifecycle or evidence record can be added after a terminal status. A setup or builder exception records `failed` before the exception is returned to the caller. The operator remains responsible for scheduling, observing and integrating the order; queue selection, serialized integration and queue-state enforcement remain outside this slice.

The same lifecycle is reachable from a shell line, because the factory line is a skill driving agents through commands rather than a TypeScript caller. The operator starts a builder with `dim worker run --role builder -- <command>`; the builder's first command is `dim order claim <order-id> --run <id> --station <name>`, under the identity carried in its environment. `dim order move <order-id> --station <name>` records that the work reached another station, and `dim order stop <order-id> <status> [--reason]` records one terminal outcome. A move carries the projection a card is read by and leaves the station it came from in the ledger, so the route an item took is readable after the fact. Evidence beyond the lifecycle has the same route: `dim order commit`, `dim order file`, `dim order check`, `dim order finding` and `dim order document` each write one row, under the rules the order contract already holds. A worker-environment report remains attached to the worker that ran the hook. The operator never claims on a builder's behalf, because the claim is the assignment and every event must name the worker that performed it.
...
- **Lifecycle.** An order is `queued`, `working`, `completed`, `dropped` or `failed`. A claim takes it straight to `working`, because an order already exists before a worker sees it and taking one is starting it. `completed` and `dropped` end it: work that stopped without landing is work nobody is holding, so a failed order is claimed again in place, and the attempts at a subject are the claims recorded against its row; a drop is the owner deciding not to build a queued order at all, carrying the reason, and it does not go back to the queue the way a failure does. A drop leaves the wall entirely rather than taking a column, because a decision not to work is none of `todo`, `active` or `done`. Both a drop and an amend of an order's own title or description are refused once anything but `queued` holds the order, since a claim already copied its words into the record and amending or dropping them afterward would leave a worker building to one statement while the queue showed another.
