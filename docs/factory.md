...
A job completes only against evidence: `completed` is refused unless a check that exited zero is recorded for the job with a timestamp no earlier than its last commit. A check that passed and was then committed over is the case the gate exists to catch, and a job with no commit recorded has nothing for its check to be older than. `blocked`, `fenced`, `failed` and `abandoned` carry no such requirement, because they are how an unchecked job stops and refusing them would push a driver toward recording the wrong outcome. The refusal carries a `job_not_checked` code and names both the command that records a check and the statuses that do not need one.

Where the gate sits has consequences. A shell caller that refuses to stop keeps the job running, so recording the check and stopping again is the way through; a builder under the driver that returns `completed` with no check recorded is failed instead, because the gate fires inside the driver's own error path and a terminal status takes no further event. A check recorded from the shell carries the moment it was recorded as its finish time, which for a loop that checks and then commits is later than the check actually ran — the gate is what forces that order, and those timestamps say when the row was written rather than how long a job spent verifying. And the gate has no skip: `DIM_SKIP_CHECK` exists for the pre-commit hook, where git's own `--no-verify` would otherwise take the subject gate with it, and the argument for this gate is that on 2026-09-18 a lane used that variable to bypass a check with nothing refusing it, and only `dim q slices` said so afterward. A driver supplying its own timestamps can still put a check where it likes, so the gate holds against forgetting rather than against a caller that means to get around it.
