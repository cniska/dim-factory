...
Planning keeps one assignment, worker and provider session in `factory_order_worker`. A later planning turn resumes that provider session and receives a rotated capability for the same worker name. An assignment that has not bootstrapped may be renewed without changing its order or role, so a harness-start failure does not create a second planner identity.
