...
The operator resolves its identity with `dim operator`, which drains hook events for the current checkout and uses its sole active session not already assigned to a station worker. If several such sessions are active, it refuses rather than selecting one by caller-supplied ID.
