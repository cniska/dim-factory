...
The turn fails, with the reason carried into the next turn's brief, when the tree cannot be committed as the builder left it — among them a red check, a check that changed the tree, a repository nested in the worktree, HEAD moved off the order's branch or past its recorded commit, and a commit git refuses. Only a check an `operator` recorded counts as build evidence, and it is always the whole declared check, whatever the order's non-goals.
...
Each adapter maps capabilities to its own sandbox. Codex runs a worker with `edit-files` under `workspace-write` and every other worker `read-only`, passing `dim`'s data directory and the checkout's git metadata with `--add-dir` to every worker, on start and on resume; on resume they go before `exec` as global flags, since `codex exec resume` takes neither. Claude runs every worker, on start and on resume, with Claude Code's Bash sandbox enabled and required to start, so a sandboxed command needs no prompt and no command can ask to run outside it; a command the user's own settings exclude from the sandbox still runs outside it. A Claude worker with `edit-files` accepts edits and is given the git metadata as well as the data directory, so it can commit from a worktree. Two sets of paths there are denied to it. The first decides what the operator's own git runs and shows, unsandboxed, the next time it touches the repository: the common config, `config.worktree`, hooks and `info/`, the replace refs and shallow list, the worktree's `.git` pointer, every linked worktree's `commondir`, `gitdir` and `config.worktree` including one added while the builder runs, and the submodules' git dirs (`protectedGitPaths` in [`src/git-metadata-dirs.ts`](../src/git-metadata-dirs.ts)). The second decides what the trunk holds: the packed refs, `refs/remotes/origin/HEAD` and, where the repository states a trunk, the trunk's own ref (`trunkRefPaths` in [`src/trunk.ts`](../src/trunk.ts)); these are file paths, so they hold under git's files ref storage and not under reftable. Neither set closes every way a builder reaches the operator, which `worker-trust-boundary` in [`build-order.md`](build-order.md) lists. A review reads the order's worktree without running a status inside a repository nested in it, while still refusing a submodule whose commit moved; every other Claude worker has the editing tools denied, its checkout denied to the sandbox, and only the data directory added.
