...
The name is issued rather than discovered. A shell becomes a worker through `dim worker mint --role <r>`, which reads the harness session from `DIM_SESSION_ID` when the harness exposes it or from the one active `SessionStart` hook recorded for the current checkout. More than one active session or no recorded session is refused, so a caller cannot guess which harness it represents; the session is unique, so minting cannot create a second identity for that harness session. A station that starts a child first creates a one-use `dim worker assign --role <r>` assignment. The child harness receives that assignment, starts with its own session, and runs the internal `dim worker bootstrap <assignment-id>` protocol; only then does the factory create the worker and record the direct parent. The child receives no parent worker token, and no worker action is accepted before the child identity exists. The worker name and token then reach the process in `DIM_WORKER_NAME` and `DIM_WORKER_TOKEN`, with only the token digest stored. The token is what makes the name a claim only its holder can make, the issued set being readable through `dim sql`.
