...
The planner and the reviewer already spawn harness-agnostically: each asks for a capability set rather than a harness's flags, and `spawn.json` turns that set and a tier's model into one harness's argv ([`src/spawn-profile.ts`](../src/spawn-profile.ts)). The builder and the operator are not spawned through this mechanism yet, and the wider operator boundary remains planned: Codex, Claude Code, Claude Desktop or another local harness may in time operate the same factory when it uses `dim` as the record and coordination boundary, with the active harness as the operator for that project and factory run.
