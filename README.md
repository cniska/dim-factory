...
It carries a handoff's `## Next` and, beside it, the check and format commands the repo declares, read by `src/tasks.ts` from `package.json`, `mise.toml` or a `Makefile`. Nothing else a query can answer goes in, because the block costs tokens in every session that starts here, and the test for a line is that a cold start cannot reach it for less — the declared commands are reachable, but only by opening a manifest, which costs a tool call and its output. The manifest is the checkout root's, since the line speaks for the repo rather than for whichever directory the session opened in. A repo that declares neither gets neither, a session outside any checkout gets no line at all, and where there is also no Next the block is empty.
