...
- Code here carries no comments, tool contracts aside. The comment gate ([`docs/usage.md`](docs/usage.md#install-the-shared-controls)) holds it for new and changed JS and TS lines, and the runner holds it for a factory builder's commit, though a builder can still rewrite the setting that switches it on ([`worker-credential-isolation`](docs/build-order.md)). A why goes into a name, a test that holds it, or the doc that owns the subject.
