...
- Code here carries no comments, tool contracts aside. The comment gate ([`docs/usage.md`](docs/usage.md#install-the-shared-controls)) holds it for new and changed JS and TS lines, except in a factory builder's commit, which skips the hook until [`builder-comment-gate`](docs/build-order.md) lands. A why goes into a name, a test that holds it, or the doc that owns the subject.
